...
const forEachTwoToThe = (arr) => {
 let foreachArr = [];
 arr.forEach( element => {
   foreachArr.push(Math.pow(2,element));
  });
  return foreachArr;
};
...
Write a function named mapTwoToThe that produces the same output as your forLoopTwoToThe function from challenge 1 and your
forEachTwoToThe function from challenge 2, but uses map instead of a for loop or forEach.
------------------------------------------------------------------------------------------------ */
...
Write a function that, given an array of numbers as input, uses map to return a new array where each element is either the string
...
const extractStats = (arr) => {
  return arr.map(statObject => {
    return {name: statObject.stat.name,
      total: statObject.effort +statObject.baseStat};
  })
};
